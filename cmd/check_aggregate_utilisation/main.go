...
func main() {
	var critical = flag.Float64("critical", defaultCriticalThreshold, "Report critical if used space is larger then <cpct> percent of the aggregate")
	var warning = flag.Float64("warning", defaultWarningThreshold, "Report warning if used space is larger than <wpct> percent of the aggregate")
	...
	var aggregate = flag.String("aggregate", "", "Only check this particular aggregate")
	...
	var aggregateList shared.AggregateList
	var err error
	...
	state := processAggregates(aggregateList, *warning, *critical)
