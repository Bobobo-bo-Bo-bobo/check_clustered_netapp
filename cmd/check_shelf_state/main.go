...
func main() {
	...
	var shelf = flag.String("shelf", "", "Only check this particular shelf")
	...
	var shelfList shared.ShelfList
	var err error
	...
	if *shelf != "" {
		shlRec, err := shared.GetNetAppShelfRecord(*host, *shelf, "state,serial_number,name,model", *username, *password, *cafile, *insecure, httpTimeout)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Can't get shelf information: %s\n", err)
			os.Exit(shared.Unknown)
		}
		shelfList.Records = append(shelfList.Records, shlRec)
	} else {
		shelfList, err = shared.GetNetAppShelfList(*host, "state,serial_number,name,model", *username, *password, *cafile, *insecure, httpTimeout)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Can't get shelf information: %s\n", err)
			os.Exit(shared.Unknown)
		}
	}
	state := processShelves(shelfList)
