...
func main() {
	...
	if *aggregate != "" {
		aggrRec, err := shared.GetNetAppAggregateRecord(*host, *aggregate, "state", *username, *password, *cafile, *insecure, httpTimeout)
		if err != nil {
		...
		aggregateList, err = shared.GetNetAppAggregateList(*host, "state", *username, *password, *cafile, *insecure, httpTimeout)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Can't get aggregate information: %s\n", err)
			os.Exit(shared.Unknown)
		}
	}
	state := processAggregates(aggregateList)
