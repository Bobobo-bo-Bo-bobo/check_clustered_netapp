...
func main() {
	...
	diskList, err = shared.GetNetAppDiskList(*host, "vendor,model,serial_number,name,bay,drawer,state", *username, *password, *cafile, *insecure, httpTimeout)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Can't get disk information: %s\n", err)
		os.Exit(shared.Unknown)
	}
	state := processDisks(diskList)
